import { JoinPacket, Peer, SearchPacket, DHTADT, ImagePacket } from './types'
import singleton from "./Singleton"

let HEADER_SIZE = 4;

const dissectJoinPacket = (message: Buffer): JoinPacket => {
    let SenderNameSize = new Number(parseBitPacket(message, 20, 12)).valueOf();

    let kadPacket: JoinPacket = {
...
const getJoinPacket = (joinPacket: JoinPacket): Buffer => {
...
const getSearchPacket = (searchPacket: SearchPacket): Buffer => {
...
const dissectSearchPacket = (message: Buffer): SearchPacket => {
    let bitMarker = 0;
    let SenderNameSize = new Number(parseBitPacket(message, 20, 12)).valueOf()
    bitMarker += 4;
    bitMarker += 8;
...
const getImagePacket = (imagePacket: ImagePacket): Buffer => {
...
const dissectImagePacket = (message: Buffer): ImagePacket => {
...
function bytes2string(array: any) {
    var result = "";
...
function parseBitPacket(packet: any, offset: any, length: any) {
    let number: number = 0;
...
function storeBitPacket(packet: any, value: any, offset: any, length: any) {
    // let us get the actual byte position of the offset
...
export { dissectJoinPacket, getJoinPacket, getSearchPacket, dissectSearchPacket, getImagePacket, dissectImagePacket }